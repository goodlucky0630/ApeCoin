import { Stack, FormControl, Flex, Input, FormLabel, Box, Button, SimpleGrid } from "@chakra-ui/react";
...
import { AUTO_COMPOUND_ABI, STAKING_ABI } from "../../constants/abi";
import { useSigner, useNetwork, useAccount } from "wagmi";
import { CompoundConfirmStake } from "./CompoundConfirmStake";
...
export const CompoundUserInput = ({ backToManage, isUnstaking }) => {
  ...
  const {
    ...
    isError,
    getAllowance,
  } = useApeCoinAllowance(AUTO_COMPOUND);
  const autoCompoundInstance = useMemo(
    () => new ethers.Contract(AUTO_COMPOUND, AUTO_COMPOUND_ABI, signer),
    [signer]
  );
  ...
  const deposit = async () => {
    ...
    try {
      setIsLoading(true);
      const deposit = await autoCompoundInstance.deposit(ethers.utils.parseEther(amount.toString()), address);
      ...
      if (result.status === 1) {
        ...
        await userContext.getCompoundBalance();
        await userContext.getCompoundedStakes();
        setAmount("");
      }
    } catch (error) {
      setIsLoading(false);
  ...
  const withdraw = async () => {
    ...
    try {
      setIsLoading(true);
      const unstaked = await autoCompoundInstance.withdraw(
  ...
  const invalidAmount = isUnstaking
    ? Number(amount) > Number(utils.formatEther(userContext?.compoundBalance?.toString()))
    : Number(amount) > Number(utils.formatEther(userContext?.balance?.toString()));

  const handleAmountButton = (percentage) => {
    const balance = isUnstaking
      ? utils.formatEther(userContext?.compoundBalance?.toString())
  ...
  const approve = async () => {
    await userContext.approveSpender(AUTO_COMPOUND);
    await getAllowance(AUTO_COMPOUND);
  };
    ...
    <Stack ref={ref} position="relative" spacing="0" maxH="fit-content">
      ...
      >
        <FormControl>
          ...
          {invalidAmount && (
            <Box color="red.300">
              {isUnstaking ? "Amount higher than staked amount." : "Amount higher than wallet balance."}
            </Box>
          )}
        </FormControl>
        <Flex flexDirection={{ base: "column", md: "row" }} justifyContent="space-between" gap="4">
          ...
          >
            <SimpleGrid columns={{ base: 2, sm: 4 }} gap="2">
              <AmountButton buttonText="25%" onClick={() => handleAmountButton(25)} />
              <AmountButton buttonText="50%" onClick={() => handleAmountButton(50)} />
              <AmountButton buttonText="75%" onClick={() => handleAmountButton(75)} />
              <AmountButton buttonText="MAX" onClick={() => handleAmountButton(100)} isMax={true} />
            </SimpleGrid>
            <Stack spacing={"1"}>
              <Box fontSize={"sm"}>{isUnstaking ? "$APE Staked:" : "$APE Balance:"}</Box>
              <Box fontSize={"lg"} fontWeight="700">
                {isUnstaking
                  ? formatAmount(utils.formatEther(userContext?.compoundBalance?.toString()), 6)
                  : formatAmount(utils.formatEther(userContext?.balance?.toString()), 6)}
              </Box>
            </Stack>
          </Flex>
          <Flex flexDirection="column" justifyContent="space-between" alignItems={"center"} gap="4">
            <ApproveButton
              w="full"
              isDisabled={
                invalidAmount ||
                amount === "" ||
                Number(amount) <= Number(utils.formatEther(allowance?.toString() || "0"))
              }
              isActive={Number(amount) > Number(utils.formatEther(allowance?.toString() || "0"))}
              onClick={approve}
            ...
            <StakeButton
              w="full"
              isActive={
                amount !== "" && Number(amount) <= Number(utils.formatEther(allowance?.toString() || "0"))
              }
              isDisabled={
                invalidAmount ||
                amount === "" ||
