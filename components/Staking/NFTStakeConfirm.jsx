...
export const NFTStakeConfirm = (props) => {
  const {
    close,
    contractCall,
    isUnstaking,
  ...
  const shareTwitter = () => {
    ...
    const tweet = `https://twitter.com/intent/tweet?text=I%20just%20${
      ...
      isUnstaking
        ? `from%20the%20${nftName[userContext.state.poolId]}`
        : `in%20the%20${nftName[userContext.state.poolId]}`
    ...
    >
      ...
      {!isSuccess ? (
        ...
        >
          ...
          {isUnstaking ? "from" : "in"} the {nftName[userContext.state.poolId]}{" "}
          Staking Pool.
        </Box>
      ) : (
