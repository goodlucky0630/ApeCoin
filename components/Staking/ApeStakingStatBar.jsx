...
export const ApeStakingStatBar = (props) => {
  const { usersShare, ...rest } = props;
  const { address } = useAccount();
  const userContext = useUserContext();

  const getShare = () => {
    if (!userContext.allStakes || !userContext?.allPools) return;
    const totalInPool = ethers.utils.formatEther(userContext?.allPools[0].stakedAmount);
    const userStaked = ethers.utils.formatEther(userContext?.allStakes[0].stakedAmount);
  ...
  return (
    ...
    >
      ...
      <Stat
        title="Pool APY"
        borderRight={{ base: "", md: "1px solid rgba(233, 233, 233, 0.3)" }}
        formattedData={userContext?.allPools && `${formatAmount(userContext?.allPools[0].apy, 4)}%`}
      />
      <Stat
        title="Your Share"
        willChangeColor={true}
        changeColor={address !== undefined}
        formattedData={address && userContext?.allPools ? getShare() : "0.00%"}
      />
    </Flex>
  );
};
